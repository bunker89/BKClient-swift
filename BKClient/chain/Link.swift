...
public class WorkLink : NetLink {
    final override func receive(connection: PeerConnection, data: [UInt8], sequence: Int) {
        ...
        do {
            ...
            let resultBool:Bool = dic[PacketConstants.RESULT] as! Bool;
            receiveJSON(jsonDic: dic, result:resultBool);
            result(res: resultBool);
        } catch {
            BKLogging.logging("WorkLinkErr: json parsing err");
            BKLogging.logging(string)
            result(res: false);
        }
    }
    ...
    final override func chainning(connection: PeerConnection, _ sequence: Int) {
        var dic = getJSONDic();
        dic[PacketConstants.WORK] = getWorking();
        ...
        do {
            ...
            BKLogging.logging("WorkLinkErr:chainning json err");
            BKLogging.logging(dic);
        }
    }
