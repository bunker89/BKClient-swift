...
class SSLSecure : Secure {
    ...
    func dataPerform() -> Bool {
        ...
        BKLogging.logging("data perform");
        ...
        if processed < 0 || processed > 1024 * 10 {
            BKLogging.logging("err processed:" + String(processed));
            return false;
        }
        
        let array:[UInt8] = Array(UnsafeBufferPointer(start:data, count:processed));
        BKLogging.logging("data perform end " + String(processed) + ", " + String(io!.avaliable()));
        if processed < 1 {
            return false;
        }
